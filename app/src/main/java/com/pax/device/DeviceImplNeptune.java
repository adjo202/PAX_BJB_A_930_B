...
/**
 * Created by wangyq on 2017/7/18.
 */
...
public class DeviceImplNeptune implements IDevice {
    ...
    private TickTimer tickTimer = new TickTimer(60, 1);
    ...
    private DeviceImplNeptune() {
        ...
        picc =  FinancialApplication.getDal().getPicc(EPiccType.INTERNAL);
        mag =  FinancialApplication.getDal().getMag();
    }
    ...
    public int pedVerifyPlainPin(byte[] iccRespOut, byte mode) {
        try {
            ped.setKeyboardLayoutLandscape(!Utils.isScreenOrientationPortrait(ActivityStack.getInstance().top()));
