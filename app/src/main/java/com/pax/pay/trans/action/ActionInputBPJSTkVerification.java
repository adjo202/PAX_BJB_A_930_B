...
import com.pax.pay.trans.action.activity.InputBPJSTkVerificationActivity;
import com.pax.settings.SysParam;

public class ActionInputBPJSTkVerification extends AAction {
    ...
    public ActionInputBPJSTkVerification(ActionStartListener listener, int infoType) {
        super(listener);
        this.infoType = infoType;
    }

    public ActionInputBPJSTkVerification(Handler handler, int infoType, ActionStartListener listener) {
        super(listener);
        this.handler = handler;
        this.infoType = infoType;
    }

    public ActionInputBPJSTkVerification(Handler handler, int infoType, ActionStartListener listener, int trans) {
        super(listener);
        this.handler = handler;
        this.infoType = infoType;
        this.trans = trans;
    ...
    public ActionInputBPJSTkVerification setParam(Context context, Handler handler, String title) {
    ...
    public ActionInputBPJSTkVerification setInfoTypeSale(String prompt, EInputType inputType, int maxLen, boolean isVoidLastTrans) {
        return setInfoTypeSale(prompt, inputType, maxLen, 0, isVoidLastTrans);
    }

    public ActionInputBPJSTkVerification setInfoTypeSale(String prompt, EInputType inputType, int maxLen, int minLen,
    ...
    public ActionInputBPJSTkVerification setIntypeAuth(String prompt, EInputType inputType, int maxLen) {
        return setIntypeAuth(prompt, inputType, maxLen, 0);
    }

    public ActionInputBPJSTkVerification setIntypeAuth(String prompt, EInputType inputType, int maxLen, int minLen) {
        this.prompt2 = prompt;
    ...
    class InputTransDataRunnable implements Runnable {
        @Override
        public void run() {
            //Sandy : just clean up the data first
            FinancialApplication.getSysParam().set(SysParam.BPJS_REGISTER_REQUEST_DATA,null);

            Intent intent = new Intent(context, InputBPJSTkVerificationActivity.class);
