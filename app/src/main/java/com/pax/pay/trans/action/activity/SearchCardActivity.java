...
public class SearchCardActivity extends BaseActivityWithTickForAction implements OnClickListener {
        ...
        switch (msg.what) {
            case READ_CARD_OK:// 读卡成功
                ...
                if (pollingResult.getReaderType() == EReaderType.MAG) {
                    //Sandy : Add (OTHTC_EMV_OPR) EMV input method optional pop-up window here
                    //add Fall back treatment
                    if ((SearchMode.INSERT == (mode & SearchMode.INSERT)) && TrackUtils.isIcCard(pollingResult.getTrack2())) {
                        ToastUtils.showMessage(SearchCardActivity.this, getString(R.string.prompt_ic_card_input));
                    ...
                    edtCardNo.setFocusable(false);
                    keyboardUtil.hideKeyBoardForBack();
                    finish(new ActionResult(0, new CardInformation(SearchMode.INSERT)));
            ...
            case READ_CARD_CANCEL:
                ...
                break;
            case READ_CARD_ERR:
                ToastUtils.showMessage(SearchCardActivity.this, getString(R.string.prompt_swipe_failed_please_retry));
                new SearchCardThread().start();
                break;
            case KEYBOARD_GONE:
                keyboardUtil.hideKeyBoardForBack();
            ...
            case EDITTEXT_DATE_ERR:
                ToastUtils.showMessage(SearchCardActivity.this, getString(R.string.prompt_card_date_err));
                edtDate.requestFocusAndTouch(keyboardUtil);
                break;
            case EDITTEXT_CARDNO_ERR:
                ToastUtils.showMessage(SearchCardActivity.this, getString(R.string.prompt_card_num_err));
                edtCardNo.requestFocusAndTouch(keyboardUtil);
                break;
            default:
                break;
