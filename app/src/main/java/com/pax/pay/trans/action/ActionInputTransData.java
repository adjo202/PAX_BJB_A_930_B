...
public class ActionInputTransData extends AAction {
    ...
    public ActionInputTransData(ActionStartListener listener, int infoType) {
        super(listener);
        this.infoType = infoType;
    }

    public ActionInputTransData(Handler handler, int infoType, ActionStartListener listener) {
        super(listener);
        this.handler = handler;
        this.infoType = infoType;
    }

    public ActionInputTransData(Handler handler, int infoType, int state,ActionStartListener listener) {
        super(listener);
        this.handler = handler;
        this.infoType = infoType;
        this.state = state;
    ...
    public ActionInputTransData setParam(Context context, Handler handler, String title) {
    ...
    public ActionInputTransData setInfoTypeSale(String prompt, EInputType inputType, int maxLen, boolean isVoidLastTrans) {
        return setInfoTypeSale(prompt, inputType, maxLen, 0, isVoidLastTrans);
    }

    public ActionInputTransData setInfoTypeSale(String prompt, EInputType inputType, int maxLen, int minLen,
    ...
    public ActionInputTransData setIntypeAuth(String prompt, EInputType inputType, int maxLen) {
        return setIntypeAuth(prompt, inputType, maxLen, 0);
    }

    public ActionInputTransData setIntypeAuth(String prompt, EInputType inputType, int maxLen, int minLen) {
        this.prompt2 = prompt;
    ...
    class InputTransDataRunnable implements Runnable {
        ...
        public void run() {
           ...
           //Log.d(TAG,"Sandy.ActionInputTransData.InputTransDataRunnable " + infoType );
            if (infoType == INFO_TYPE_SALE) {
            ...
            } else if (infoType == INFO_TYPE_AUTH) {
                ...
                throw new IllegalArgumentException("unsupported argument infoType: " +
                        infoType);
            }
        }
    }
