...
public class DetailsActivity extends BaseActivity implements OnClickListener, OnItemOnClickListener {
    ...
    protected void initViews() {
        ...
        // add abdul detail pulsa data
        if (transType == ETransType.PURCHASE_PULSA_DATA.toString() || transType == ETransType.INQ_PULSA_DATA.toString()) {
            confirmBtn.setText("CEK STATUS");
        }
        ...
        RelativeLayout layer = new RelativeLayout(DetailsActivity.this);
        layer.setLayoutParams(params);

        if (Utils.isScreenOrientationPortrait(DetailsActivity.this)) {
            for (int i = 0; i < titles.size(); i++) {
                layer = ViewUtils.genSingleLineLayout(DetailsActivity.this, titles.get(i), values.get(i));
    ...
    public void onItemClick(ActionItem item, int position) {
        ...
        if (title.equals(getString(R.string.trans_reprint))) {
            ...
            new Thread(new Runnable() {
                @Override
                public void run() {
                    Printer.printTransAgain(DetailsActivity.this, handler, transData);
                }
            }).start();
        }
        // 撤销
        if (title.equals(getString(R.string.trans_void))) {
           ...
           // Log.d(TAG,transType);
            ...
            if (transType.equals(ETransType.SALE.toString())) {
                new SaleVoidTrans(DetailsActivity.this, handler, transData, null).execute();
            } else if (transType.equals(ETransType.COUPON_SALE.toString())) {
                new CouponVoidTrans(DetailsActivity.this, handler, transData, null).execute();
            }else if (transType.equals(ETransType.AUTHCM.toString())) {
                // 预授权完成请求撤销
                new AuthCMVoidTrans(DetailsActivity.this, handler, transData, null).execute();
            } else if (transType.equals(ETransType.QR_SALE.toString())) {
                // 扫码消费撤销
                new QrSaleVoidTrans(DetailsActivity.this, handler, transData, null).execute();
            } else if(transType.equals(ETransType.DANA_QR_SALE.toString())){
                new DanaSaleVoidTrans(DetailsActivity.this,handler,transData,null).execute();
            }
        }
        // 预授权完成通知
        if (title.equals(getString(R.string.auth_cm_adv))) {
            new AuthSettlementTrans(DetailsActivity.this, handler, transData, true, false, null).execute();
        }
        // 预授权完成请求
        if (title.equals(getString(R.string.auth_cm_req))) {
            new AuthCMTrans(DetailsActivity.this, handler, transData, true, false, null).execute();
        }
    }
    ...
    private void initialList() {
        ...
        String formater;
        if (transType.equals( ETransType.INQ_PULSA_DATA )){
            formater = FinancialApplication.getConvert().amountMinUnitToMajor(
